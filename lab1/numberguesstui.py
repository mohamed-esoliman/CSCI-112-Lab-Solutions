...
def main(low=1, high=100):
    ...
    # modified: calculated max tries based on range
    max_tries = math.ceil(math.log((low + high - 1), 2))
    count = 0
    while True:
        count += 1
        ...
        # modified: added max tries check
        if count == max_tries:
            print(
                "You have exceeded the maximum number of tries! The number was %d."
                % number
            )
            break
